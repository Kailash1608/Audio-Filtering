...
\author{Kailash.C\\EE23BTECH11207}
\date{}
...
\begin{align}
    ...
    j&=9
\end{align}
 substituting $j =9$ gives the passband
range for our bandpass filter as $9.4$ kHz - $10.6$ kHz.  Hence, the un-normalized discrete time filter
passband frequencies are $F_{p1} = 9.4$ kHz
and $F_{p2} = 10.6$ kHz. \\
The corresponding normalized digital filter passband frequencies are
\begin{align}
    \omega_{p1} &= 2\pi\frac{F_{p1}}{F_s} = 0.39 \pi\\
    \omega_{p2} &= 2\pi\frac{F_{p2}}{F_s}  =0.44 \pi
\end{align}
...
\begin{align}
    F_{s1} &= 9.4-0.3 = 9.1 \text{KHz}\\
    F	_{s2} &= 10.6+0.3 = 10.9  \text{KHz}
\end{align}
\begin{align}
    \omega_{s1} &= 2\pi\frac{F_{s1}}{F_s} = 0.379 \pi\\
     \omega_{s2} &= 2\pi\frac{F_{s2}}{F_s} = 0.454 \pi\\
\end{align}
\end{enumerate}
...
$\Omega_{p1} = 0.703$, $\Omega_{p2} = 0.827$ and $\Omega_{s1} = 0.677$, $\Omega_{s2} = 0.865$
respectively.
...
\end{equation}
where $\Omega_0 = \sqrt{\Omega_{p1}\Omega_{p2}} = 0.762$ and $B = \Omega_{p2} - \Omega_{p1} = 0.124$.

Substituting $\Omega_{s1}$ and $\Omega_{s2}$ in \eqref{eq:freq_transform} we obtain the stopband edges of lowpass filter 
\begin{align}
    \Omega_{Ls1} &= \frac{\Omega_{s1}^2 - \Omega_0^2}{B\Omega_{s1}} = -1.467\\
    \Omega_{Ls2} &= \frac{\Omega_{s2}^2 - \Omega_0^2}{B\Omega_{s2}} = 1.562
\end{align}
And we choose the minimum of these two stopband edges
\begin{align}
    \Omega_{Ls} = \mbox{min}(\vert \Omega_{Ls_1}\vert,\vert \Omega_{Ls_2}\vert) = 1.457.
\end{align}
...
\begin{lstlisting}
https://github.com/Kailash1608/Audio-Filtering/blob/main/Filter_Design/Codes/plot1.py
\end{lstlisting}
\begin{figure}[H]
\centering
...
\begin{lstlisting}
https://github.com/Kailash1608/Audio-Filtering/blob/main/Filter_Design/Codes/sk_gen.c
\end{lstlisting}
...
\begin{lstlisting}
https://github.com/Kailash1608/Audio-Filtering/blob/main/Filter_Design/Codes/pole_zero_plt.py
\end{lstlisting}
\begin{figure}[H]
\centering
